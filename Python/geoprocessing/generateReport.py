...
def genSummaryReport(masterTable, summaryTable):
    ...
    arcpy.Statistics_analysis(masterTable, summaryTable, [['Name', 'First'], ['Levy', 'SUM']], ['District_ID'])

    expression = "{} LIKE '{}'".format(arcpy.AddFieldDelimiters(summaryTable, 'FIRST_Name'), '%')
    selectAllExpression = "{} LIKE '{}'".format(arcpy.AddFieldDelimiters(masterTable, 'APN'), '%')

    with arcpy.da.SearchCursor(summaryTable, ['District_ID', 'FIRST_Name', 'FREQUENCY', 'SUM_Levy'],
    ...
    with arcpy.da.SearchCursor(masterTable, ['APN', 'District_ID', 'Levy'],
        ...
        for selectRow in selectAllCursor:
            ...
            corrections = u"      {} {}  {}<br>".format(apn, disID, levy)
            print (corrections)
            print type(corrections)
            test = corrections.encode('ascii', 'replace')
            print test
            print type(test)
...
csa_SummaryTable = r"Database Connections\Connection to SQLRIAGD01.rivcoca.org (2).sde\GDB_EDA.EDA.CSA_Summary"
...
if len(getGenerateValue) == 1:
    ...
    created_date = arcpy.AddFieldDelimiters(master_18_19, 'last_edited_date')
    ...
    arcpy.SetParameterAsText(1, apnList)
    arcpy.SetParameterAsText(2, json.dumps(apn_Dict))
